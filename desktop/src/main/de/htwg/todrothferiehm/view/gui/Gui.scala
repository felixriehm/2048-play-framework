package de.htwg.todrothferiehm.view.gui
...
import de.htwg.todrothferiehm.model._
...
class Gui(val controllers: ControllerContainer) extends JFXApp with Reactor {
  ...
  var task = new java.util.TimerTask {
    def run() = AiConfiguration.ai.step(controllers)
  }
  ...
  def ScoresGaine(scoreGain: Int): Unit = {
    Platform.runLater({val timeline = new Timeline {
      cycleCount = 1
      autoReverse = false
      ...
      keyFrames = Seq(
        at (350 ms) {addedScore.translateYProperty() -> -30 tween Interpolator.LINEAR},
        at (350 ms) {addedScore.opacityProperty() -> 0 tween Interpolator.LINEAR}
      )
    }
      timeline.play()})
  }
  ...
  def MergeTile(pos: (Int, Int), number: Int): Unit = {
    Platform.runLater({val timeline = new Timeline {
      ...
      keyFrames = Seq(
        at (100 ms) {addedCell.scaleXProperty() -> 1 tween Interpolator.LINEAR},
        at (100 ms) {addedCell.scaleYProperty() -> 1 tween Interpolator.LINEAR},
        at (275 ms) {addedCell.scaleXProperty() -> 1.2 tween Interpolator.LINEAR},
        at (275 ms) {addedCell.scaleYProperty() -> 1.2 tween Interpolator.LINEAR},
        at (450 ms) {addedCell.scaleXProperty() -> 1 tween Interpolator.LINEAR},
        at (450 ms) {addedCell.scaleYProperty() -> 1 tween Interpolator.LINEAR}
      )
    }
      timeline.play()})
  }
  ...
  def SlideTile(start:(Int,Int),end:(Int,Int)): Unit = {
    ...
    Platform.runLater({val timeline = new Timeline {
      ...
      keyFrames = Seq(
        if(start._1 == end._1){
          at (100 ms) {slideCell.layoutYProperty -> (slideCell.getLayoutY+ endcell.getLayoutY - startcell.getLayoutY) tween Interpolator.LINEAR}
        }else{
          at (100 ms) {slideCell.layoutXProperty -> (slideCell.getLayoutX+endcell.getLayoutX - startcell.getLayoutX) tween Interpolator.LINEAR}
        }
      )
    }
      timeline.play()})
  ...
  var rootScene = new VBox{
    stylesheets += this.getClass.getResource("main.css").toExternalForm
    alignment = Pos.Center
    val image = this.getClass.getResource("bg_pattern.gif").toExternalForm
    style = "-fx-background-image: url('" + image + "'); -fx-background-repeat: repeat;"
    children = List(new FlowPane {
    alignment = Pos.Center
    ...
    onMouseClicked = (k: MouseEvent) => {
      ...
      text = "Protip: Use wasd or arrow keys to move, n to start a new game."
      style = "-fx-text-fill: white; -fx-text-alignment: right; -fx-font-size: 7pt; -fx-font-family: Verdana, sans-serif; -fx-font-weight: bold; -fx-color: white;"
    },new HBox{
      alignment = Pos.Center
      children = List(new Label{
        text = "AI step"
        style = "-fx-underline: true; -fx-text-fill: white; -fx-text-alignment: right; -fx-font-size: 7pt; -fx-font-family: Verdana, sans-serif; -fx-font-weight: bold; -fx-color: white;"
        onMouseClicked = (k: MouseEvent) => AiConfiguration.ai.step(controllers)
      },new Label{
        text = "AI start/stop"
        style = "-fx-underline: true; -fx-text-fill: white; -fx-text-alignment: right; -fx-font-size: 7pt; -fx-font-family: Verdana, sans-serif; -fx-font-weight: bold; -fx-color: white;"
        onMouseClicked = (k: MouseEvent) => {
